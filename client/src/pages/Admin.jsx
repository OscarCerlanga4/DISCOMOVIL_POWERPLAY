...
const dibujarCabeceraEmpresa = (doc, empresa, margen, y, logoBase64) => {
    ...
    if (logoBase64) {
        try { doc.addImage(logoBase64, 'PNG', margen + 7, y + 5, 66, 34) }
            ...
            <div style={{ display: 'flex', marginBottom: '2.5rem', borderBottom: '1px solid rgba(255,255,255,0.07)' }}>
                {FILTROS.map(f => {
                    ...
                    return (
                        ...
                        >
                            {f.label}
                            <span style={{ marginLeft: '0.35rem', fontSize: '0.68rem', opacity: 0.55 }}>({count})</span>
                        </button>
                    )
                })}
            </div>
                    ...
                    {filtrados.map(p => {
                        ...
                        return (
                            <div key={p.id_presupuesto} style={{
                                background: '#141414',
                                borderLeft: `3px solid ${ACCENT[p.estado] || 'rgba(255,255,255,0.1)'}`
                            }}>
                                {/* Fila principal */}
                                <div style={{ padding: '1.25rem 1.75rem', display: 'flex', alignItems: 'center', gap: '1.5rem' }}>
                                    ...
                                    <div style={{ flex: 1, display: 'flex', gap: '2.5rem', flexWrap: 'wrap', alignItems: 'flex-start', minWidth: 0 }}>
                                        ...
                                        <div>
                                            ...
                                            <p style={{ color: '#fff', margin: '0.2rem 0 0', fontSize: '0.9rem', fontWeight: 600 }}>
                                                {p.reserva?.cliente_nombre || '—'}
                                            </p>
                                            <p style={{ color: 'rgba(255,255,255,0.3)', margin: '0.1rem 0 0', fontSize: '0.78rem' }}>
                                                {p.reserva?.cliente_email || ''}
                                            </p>
                                        </div>
                                        <div>
                                    ...
                                    <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center', flexShrink: 0 }}>
                                        <button
                                            onClick={() => setExpandido(abierto ? null : p.id_presupuesto)}
                                        ...
                                        <button
                                            onClick={() => generarPdfPresupuesto(p, empresa)}
                                            style={btnAmarillo}
                                            onMouseEnter={e => e.currentTarget.style.opacity = '0.8'}
                                        ...
                                        {p.estado !== 'aceptado' && p.estado !== 'rechazado' && (
                                            <>
                                                <button
                                                    onClick={() => cambiarEstado(p.id_presupuesto, 'aceptado')}
                                                    disabled={accionando === p.id_presupuesto}
                                                    style={{
                                                        background: 'transparent', border: '1px solid rgba(96,192,96,0.4)',
                                                        color: '#60c060', fontFamily: 'Bebas Neue', fontSize: '0.85rem',
                                                        letterSpacing: '0.12em', padding: '0.55rem 1.1rem', cursor: 'pointer',
                                                        opacity: accionando === p.id_presupuesto ? 0.5 : 1,
                                                ...
                                                <button
                                                    onClick={() => cambiarEstado(p.id_presupuesto, 'rechazado')}
                                                    disabled={accionando === p.id_presupuesto}
                                                    style={{
                                                        background: 'transparent', border: '1px solid rgba(255,68,68,0.35)',
                                                        color: '#ff6666', fontFamily: 'Bebas Neue', fontSize: '0.85rem',
                                                        letterSpacing: '0.12em', padding: '0.55rem 1.1rem', cursor: 'pointer',
                                                        opacity: accionando === p.id_presupuesto ? 0.5 : 1,
                                                        transition: 'all 0.2s', whiteSpace: 'nowrap', flexShrink: 0
                                                    }}
                                    ...
                                    <div style={{ borderTop: '1px solid rgba(255,255,255,0.06)', padding: '1.5rem 1.75rem', background: '#111' }}>
                                        ...
                                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '1.5rem', gap: '1rem', flexWrap: 'wrap' }}>
                                            <div>
                                                ...
                                                <p style={{ color: '#fff', margin: '0.3rem 0 0', fontSize: '0.88rem', fontWeight: 600 }}>
                                                    {p.reserva?.ubicacion || '—'}
                                                </p>
                                            </div>
                                        ...
                                        {(!p.detalle_presupuesto || p.detalle_presupuesto.length === 0) ? (
                                            ...
                                            <div>
                                                <div style={{ display: 'grid', gridTemplateColumns: '1fr 80px 130px 120px', padding: '0.4rem 0.75rem', marginBottom: '0.25rem' }}>
                                                    {['Concepto', 'Cantidad', 'Precio/hora', 'Subtotal'].map(h => (
                                                        <p key={h} style={{ ...LABEL, margin: 0 }}>{h}</p>
                                                    ))}
                                                </div>
                                                {p.detalle_presupuesto.map((d, i) => (
                                                    <div key={d.id_detalle || i} style={{
                                                        display: 'grid', gridTemplateColumns: '1fr 80px 130px 120px',
...
export default function Admin() {
    ...
    return (
        <div style={{ background: '#0d0d0d', minHeight: '100vh', paddingTop: '80px', display: 'flex' }}>
            ...
            {/* Sidebar */}
            <div style={{
                width: '220px', background: '#080808',
                borderRight: '1px solid rgba(255,230,0,0.1)',
                flexShrink: 0,
                ...
                <div style={{ padding: '2rem 1.5rem 1.25rem' }}>
                    <p style={{ fontSize: '0.55rem', fontWeight: 700, letterSpacing: '0.25em', textTransform: 'uppercase', color: 'rgba(255,230,0,0.5)', margin: '0 0 0.5rem' }}>
                    ...
                    {secciones.map(s => (
                        <button
                            ...
                            style={{
                                ...
                                background: seccionActiva === s.id ? 'rgba(255,230,0,0.05)' : 'transparent',
                                border: 'none',
                                borderLeft: seccionActiva === s.id ? '3px solid #FFE600' : '3px solid transparent',
                                padding: '0.9rem 1.5rem',
                                fontFamily: 'Bebas Neue', fontSize: '1.05rem', letterSpacing: '0.15em',
                                color: seccionActiva === s.id ? '#FFE600' : 'rgba(255,255,255,0.35)',
            ...
            <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minHeight: 'calc(100vh - 80px)' }}>
                <div style={{ borderBottom: '1px solid rgba(255,230,0,0.1)', padding: '2rem 3.5rem', background: '#0a0a0a' }}>
                    <p style={{ fontSize: '0.6rem', fontWeight: 700, letterSpacing: '0.2em', textTransform: 'uppercase', color: 'rgba(255,255,255,0.2)', margin: '0 0 0.4rem' }}>
                        Admin · {seccionLabel}
                    </p>
                    <h1 style={{ fontFamily: 'Bebas Neue', fontSize: '2.8rem', letterSpacing: '0.08em', color: '#fff', margin: 0, lineHeight: 1 }}>
                        {seccionLabel}
                    </h1>
                </div>
                ...
                <div style={{ flex: 1, padding: '2.5rem 3.5rem' }}>
                    ...
                    {seccionActiva !== 'presupuestos' && (
                        <p style={{ color: 'rgba(255,255,255,0.15)', fontFamily: 'Bebas Neue', fontSize: '1.3rem', letterSpacing: '0.1em' }}>
                            Próximamente — {seccionLabel}
                        </p>
                    )}
                </div>
            </div>
